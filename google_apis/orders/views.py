...
from telegram_bot.tg_bot import send_alarm
...
def get_data_from_google_sheets():
    ...
    result = (
        ...
        .values()
        .get(spreadsheetId=spreadsheet_id, range="A2:D", majorDimension="ROWS")
        .execute()
    )
...
def data_view(request):
    ...
    orders_list = check_delivery_time(queryset)
    text = f"У вас {len(orders_list)} просроченных заказов:\n {orders_list}"

    send_alarm(token=settings.BOT_TOKEN, chat_id=settings.CHAT_ID, text=text)
